/**
 * Canonical JSON — the two serializations Decision 2 (SPEC §2) requires:
 *
 * - `canonicalStringify` is the on-disk snapshot format: keys sorted
 *   lexicographically at every depth, 2-space indent, LF endings, trailing
 *   newline. Git-diff legibility beats byte-canonical JCS for the committed
 *   file.
 * - `jcsCanonical` is the compact sorted form hashes are computed over
 *   (probeSpecHash, text-block sha256, structural equality in diff rules).
 *   For JSON-safe data serialized by ECMAScript `JSON.stringify` number
 ...
 * coerced value would change a hash or a committed snapshot.
 */
...
/** Runtime Json validation for boundaries (wire results, snapshot fields). */
...
function serialize(value: unknown, pretty: boolean, depth: number, path: (string | number)[]): string {
  ...
  switch (typeof value) {
    ...
    case "object": {
      if (Array.isArray(value)) {
        const items = (value as unknown[]).map((v, i) => {
          if (v === undefined) {
            ...
            // hash / snapshot — reject loudly instead.
      ...
      for (const key of keys) {
        ...
        entries.push(pretty ? `${JSON.stringify(key)}: ${serialized}` : `${JSON.stringify(key)}:${serialized}`);
      }

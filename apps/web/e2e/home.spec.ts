...
test.describe("/ — the real diff (hero evidence, computed at build time)", () => {
  test("shows a real breaking finding from the committed drift-breaking@v2 fixture, with tier and exit code", async ({
    ...
    await expect(hero.getByText("breaking", { exact: true })).toBeVisible();
    await expect(hero.getByText("tool.removed")).toBeVisible();
    await expect(hero.getByText(/tool "archive_note" was removed/)).toBeVisible();
    await expect(hero.getByText(/CI would exit/)).toContainText("1");
  });
});
